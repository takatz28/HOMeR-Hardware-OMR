function [unbeamedNotes, ledgerLineLocs] = detectUnbeamedNotes(section, spaceHeight, lineHeight, wholeNotes)
...
%             imshow(noteHead); pause;
%             noteHead = imdilate(noteHead, strel('disk', 2));
%             imshow(noteHead); pause;
...
%                     elseif (numel(noteHeadStats) > 1)
                    ...
                    noteHead2 = imdilate(noteHead, strel('disk', floor(spaceHeight*0.3)));                        
%                         imshow(xor(~noteHead2, tempImg)); pause;
                    filled1 = imopen(xor(~noteHead2, tempImg), strel('line', floor(spaceHeight*1.25), -45));
%                         imshowpair(section, filled1); pause;
                    cc1 = bwconncomp(filled1);
                    flagStats1 = regionprops(cc1, 'Area');
                    flagCount = numel(flagStats1);
%                         filled1 = ismember(labelmatrix(cc1), ...
%                             find([flagStats1.Area] <= 15*spaceHeight));
%                         flagCount = numel(regionprops(filled1));
                    if(flagCount == 0)
                        filled2 = imopen(xor(~noteHead2, tempImg), strel('line', floor(spaceHeight*1.25), 45));
%                             imshowpair(section, filled2); pause;
                        cc2 = bwconncomp(filled2);
                        flagStats2 = regionprops(cc2, 'Area');
                        flagCount = numel(flagStats2);
%                             filled2 = ismember(labelmatrix(cc2), ...
%                                 find([flagStats2.Area] <= 15*spaceHeight));
%                             flagCount = numel(regionprops(filled2));
%                             imshowpair(section, filled2); pause;
%                         else
%                             imshowpair(section, filled1); pause;
%                             imshow(filled2); pause;
                    end
%                         flagCount
%                         pause
%                         imshow(filled1); pause;
%                         filled = imerode(filled, strel('square', 2));
%                         imshow(~xor(filled,tempImg)); pause;
                    %{
                    flagStem = imerode(~xor(filled,tempImg), ...
                        strel('rectangle', [floor(spaceHeight/2.5), ...
                        floor(spaceHeight/3)]));
%                         imshow(~flagStem); pause;
                    flags = imclose(flagStem,strel('line',10,0));
%                     imshow(~flags); pause;
